...
impl GFXDevice {
    ...
    // build pipeline if needed and bind it to the current cmd
    fn build_pipeline(&self, cmd: &CommandBuffer) {
        let pipeline_state = cmd.pipeline_state;

        let vertex_input_attribute_descriptions = cmd
            .pipeline_state
            .pipeline_desc
            .vertex_input_attribute_descriptions;
        let vertex_input_binding_descriptions = cmd
            .pipeline_state
            .pipeline_desc
        ...
        // self.device
        //     .cmd_bind_pipeline(cmd.cmd, vk::PipelineBindPoint::GRAPHICS, pipeline);
    }
    ...
    pub fn draw_indexed(
        ...
        unsafe {
            ...
            self.device
                .cmd_draw_indexed(cmd.cmd, index_count, 1, 0, 0, 1);
        }
    }
    ...
    pub fn bind_pipeline(&self, pipeline_state: &PipelineState, cmd: &CommandBuffer) {
    ...
    pub fn create_pipeline_state(&self, desc: &PipelineStateDesc) -> PipelineState {
        unsafe {
            ...
            if let Some(ref vertex_shader) = desc.vertex {
                ...
                set_layouts.push(vertex_shader.desc_set_layout);
                push_constant_ranges.extend(vertex_shader.push_constant_ranges);
            };
            ...
            if let Some(ref fragment_shader) = desc.fragment {
                ...
                set_layouts.push(fragment_shader.desc_set_layout);
                push_constant_ranges.extend(fragment_shader.push_constant_ranges);
            };
        ...
        unsafe {
            match spirv_reflect::create_shader_module(byte_code) {
                Ok(module) => {
                    ...
                    let mut set_layout_bindings = Vec::with_capacity(bindings.len());
