...
VsOut main(uint vertex_index:SV_VertexID , uint instance_index:SV_InstanceID){
    ...
    float3 normal =asfloat(vertices.Load3(mesh.normal_offset + vertex_index*sizeof(float3)));
    
    float4 tangent_packed =asfloat(vertices.Load4(mesh.tangents_offset + vertex_index*sizeof(float4)));
 ...
 //   
  //  vs_out.normal =vs_out.normal;// normalize(mul(model_transform, float4(,0.0)).xyz);
    vs_out.tangent = tangent_packed.xyz;//normalize(mul(model_transform, float4(,0.0)).xyz);
    //
    vs_out.bitangent = (cross(normal, vs_out.tangent)*tangent_packed.w); 
  //  vs_out.bitangent = normalize(mul(model_transform, float4(vs_out.bitangent,0.0)).xyz);
    //
